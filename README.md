...
We assume an installation of VMD 2.0.0a9 from the binary bundle `vmd-2.0.0a9.bin.LINUXAMD64.tar.gz` within `$HOME/opt/vmd-2.0.0a9`.

This installation has been generated by modifying the `./configure` file in the package root and running `make install` from within the `src` subfolder.

The following walks through rebuilding the VMD plugins of VMD 1.9.4a57 on Ubuntu 24.04, with `pbctools` and `topotools` replaced with their current development versions.

We assume source code extracted from `vmd-1.9.4a57.src.tar.gz`.

Ubuntu 24.04 ships with `tcl8.6` instead of `tcl8.5`. We have to replace occurences in the Makefiles within `vmd-1.9.4a57.src/plugins` with

    grep -rl -- "tcl8.5" . | xargs sed -i 's/tcl8\.5/tcl8.6/g'

Remove original folders within `vmd-1.9.3.src/plugins` with
...
and repace them with
    ...
    git clone https://github.com/frobnitzem/pbctools.git pbctools
    git clone https://github.com/NanoCIPHER-Lab/topotools.git topotools

while adding

    git clone https://github.com/jotelha/mergetools.git mergetools

as well. Also add the entry `mergetools` to the `BUILDDIRS` variable in the `Makefile`.
...
Next, we specify where to install the plugins,

    export PLUGINDIR=$HOME/opt/vmd-1.9.4a55-plugins

and run

    make distrib

We also assume the `tcllib` 2.0 source code extracted from `tcllib-2.0.tar.gz`, and build from within the source directory with

    ./configure --prefix=$HOME/opt/tcllib-2.0/
    make install

or some other custom directory.

Eventually, we run VMD with environment variables pointing to the custom `tcllib` and plugins directory.

    TCLLIBPATH="$HOME/opt/tcllib-2.0" VMDPLUGINPATH="$HOME/opt/vmd-1.9.4a55-plugins/LINUXAMD64:$HOME/opt/vmd-1.9.4a55-plugins/noarch" vmd

Confirm all desired packages are loadable, i.e. with
